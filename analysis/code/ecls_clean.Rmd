...
```{r}
ecls_clean <- ecls_clean %>%
  mutate(class_size = a5girls + a5boys) %>%
  mutate(treatment3 = as.numeric(class_size <= 26))

mod <- lm(c6r4mscl ~ treatment3, data = ecls_clean)
summary(mod)
```
...
ah <- ecls_new %>%
  clean_names() %>%
  mutate(class_size_k = pmax(a2aenrol, a2penrol, a2denrol, na.rm = T), 
         class_size_1 = pmax(a4enrol, na.rm))


ecls_new %>%
  dplyr::select(A4ENROL)
```
