...
ecls_clean <- ecls_clean %>%
  mutate(
    ...
    across(
      matches("^[gntm]7(behind|tlkout|passiv|relwel|disrup|absent|tardy|honors|organz|grammar|gather|wrtvar|style|analyo|creato|attent|orgdat|write|problm|presen|designn|scconc|scprob|talkab|proofs|apply|repres)"),
      ~ if_else(.x <= 0, NA_real_, .x)
    ),
...
ecls_clean <- ecls_clean %>%
  mutate(
    ...
    tch_cog_7 = rowMeans(pick(matches("^[gnm]7(behind|organz|grammar|gather|wrtvar|style|analyo|creato|orgdat|write|problm|presen|design|scconc|scprob|talkab|proofs|apply|repres)_z")), na.rm = TRUE), 
    tch_g_7 = rowMeans(pick(matches("^[gnm]7(grammar|wrtvar|style|analyo|creato)_z")), na.rm = TRUE)
  )
...
ecls_clean <- ecls_clean %>%
  mutate(
    ...
    tch_cog_6 = rowMeans(pick(matches("^[gnm]6(alg|rtmth|abil|divide|strat|plcvl|shape|rtlang|abil|promot|speak|raloud|info|rdfln|expost|mltstr|reflwr|rtskil|expln|clssfy|comsc|sciprd|princp)_z")), na.rm = TRUE)
  )
...
covar_list <- "p1hfamil + w1raceth + wkincome + p1hmemp + p1hdemp + p1numsib + w1momed + w1daded + w1dadscr + w1momscr + p1firkdg + p1disabl + sch_math_mean_7"

# + sch_math_mean_1 + sch_rdg_mean_1 + p1firkdg + 

covar_list <- "p1hfamil + wkincome + p1hmemp + p1hdemp + p1numsib + w1momed + w1daded + w1dadscr + w1momscr + p1firkdg + p1disabl + sch_ageent_mean_1"

model_honors <- run_iv("c4r4mscl", paste0("age_test_7 +", covar_list))
