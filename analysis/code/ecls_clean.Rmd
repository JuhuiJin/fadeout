...
# Impact of Parent and Teacher's Positive Distortionary Beliefs
```{r}
for (i in 1:7) {
...
plot <- ecls_clean %>%
  ...
  labs(
    ...
    y = "Mean Kindergarten Entry Age (Months)", 
...
# Verifying that the 1999 entries are basically bogus past 3
# How to justify this measurement error? 
ecls_clean %>%
  filter(s2yyfive == 1999 & s2mmfive >= 1) %>%
  filter(p1ageent <= 50) %>%
  select(s2mmfive, s2yyfive, p1ageent, s1_id)

ecls_clean %>%
  filter(s2yyfive == 1999 & s2mmfive >= 5) %>%
  select(s2mmfive, p1ageent) %>%
  filter(p1ageent <= 56)
...
# RD of Students just over the threshold 
```{r}
ecls_clean %>%
  filter(p1ageent >= 0) %>%
  group_by(s2mmfive) 
...
ecls_clean %>%
  select("dobmm", "dobdd", "dobyy", "p1firkdg", "s2mmfive", "s2ddfive", "s2yyfive", "p1ageent")

jk <- ecls_clean %>%
  mutate(
    before = case_when(
      dobmm >= 7 & dobmm <= s2mmfive & dobdd <= s2ddfive ~ 1, 
      dobmm == s2mmfive & dobdd >= s2ddfive | dobmm > s2mmfive ~ 0, 
      dobmm <= 7 ~ 7,
      TRUE ~ NA_real_
    )
  ) %>%
  mutate(
    dob_numeric = dobmm + (dobdd / 12)
  ) 

# Check balance in kids before and after cutoff 
jk %>%
  group_by(before) %>%
  summarize(
    mean_ageent = mean(p1ageent, na.rm = T), 
    med_ageent = quantile(p1ageent, 0.5, na.rm = T), 
    mean_incomek = mean(wkincome, na.rm = T), 
    mean_income1 = mean(w1income, na.rm = T), 
    mean_dadscr = mean(w1dadscr, na.rm = T), 
    mean_momscr = mean(w1momscr, na.rm = T), 
    mean_povrty = mean(as.numeric(w1povrty), na.rm = T),
    mean_school_math = mean(sch_math_mean_1, na.rm = T), 
    mean_daded = mean(as.numeric(w1daded), na.rm = T), 
    mean_momed = mean(as.numeric(w1momed), na.rm = T), 
    mean_math_1 = mean(c1r4mscl, na.rm = T),
    n = n()
  )

# The difference-in-means gap between older and younger children shrinks by 8th grade. 
# Fade-out explained by specific skills / item level measures? 

sum(is.na(ecls_clean$dobyy))
...
ecls_clean %>%
  filter(s2mmfive >= 0) %>%
  filter(p1ageent >= 0) %>%
  filter(!is.na(s1_id) & !is.na(s4_id)) %>%
  filter(s2yyfive == 1998) %>%
  group_by(s2mmfive) %>%
  summarize(
    mean = mean(p1ageent, na.rm = T), 
    n = n()
  ) %>%
  ggplot(aes(x = s2mmfive, y = mean)) + 
  geom_line() + 
  theme_minimal()

# As they should be, birthdates are basically random 
print(jk)

# Check if you have differential paper 

date = as.Date(paste(s2yyfive, s2mmfive, "01", sep = "-"))
head(ecls)


table(ecls_clean$s2yyfive)

ecls_clean %>%
  filter(s2yyfive == 1999) %>%
  summarize(
    n = n_distinct(s1_id)
  )
```
...
# Using School's Required Entry rules as an instrument 
```{r}
ecls_clean %>%
  select(matches("five"))

summary(ecls_clean$s2mmfive)
...
jk <- ecls_clean %>%
  ...
  group_by(s4_id) %>%
  mutate(
    u4schbmm = first(u4schbmm[!is.na(u4schbmm)])
  ) %>%
  group_by(s2mmfive, u4schbmm) %>%
  summarize(
    mean = mean(p1ageent, na.rm = T), 
    n = n()
  ) 
...
print(jk)
  ...
  ggplot(aes(s2mmfive, y = mean)) + 
  geom_line() + 
  theme_minimal()

jk <- ecls_clean %>%
  select(s4_id, s6_id, s7_id,s5_st_id, s6_st_id) %>%
  distinct()

ecls_clean %>%
  filter(s1_id != s4_id) %>%
  filter(!s1_id %in% jk$s4_id) %>%
  filter(!is.na(t4_id)) %>%
  group_by(s1_id, s4_id) %>%
  summarize(n = n() - sum(is.na(s4_id))) %>%
  filter(n == 1)
...
ecls_clean %>%
  ...
  filter(!s1_id %in% jk$s4_id) %>%
  group_by(s1_id) %>%
  summarize(n = n_distinct(s4_id)) %>%
  filter(n == n() - sum(is.na(s1_id)))
...
# Entry Rules are a Good Instrument!
ecls_clean %>%
  filter(s2mmfive >= 0) %>%
  filter(p1ageent >= 0) %>%
  filter(!is.na(s1_id) & !is.na(s4_id)) %>%
...
ggplot(aes(x= s2mmfive, y = mean), data = jk) + 
  geom_line() + 
  theme_minimal()
...
jk <- ecls_clean %>%
  filter(s2yyfive == 1998) %>%
  filter(s2mmfive >= 0) %>%
  mutate(after_7 = as.factor(s2mmfive >= 7)
  ) 
```
...
# And also t6learn! 
# As well as M6RTMTH 
model <- lm(p1ageent ~ p1hfamil + w1raceth + w1momed + w1daded + s2mmfive * after_7 + p1firkdg + sch_math_mean_1, data = jk)
summary(model)
...
for (i in c(1,2,4,5,6)) {
  # Teacher Vars
  assign(paste0("tch_eval_", i), c(paste0("t", i, "learn"), paste0("t", i, "extern"), paste0("t", i, "intern"), paste0("t", i, "contro"), paste0("t", i, "difliv")))
...
for (i in c(1,4,5,6,7)) {
  # Parent Evaluations 
  assign(paste0("par_eval_", i), c(paste0("p", i, "sameag"), paste0("p", i, "learn"), paste0("p", i, "social")))
}

test_eval_7 <- c("c7r4rscl", "c7r4mscl", "c7r2sscl")
tch_eval_cog_7 <- c("t7arsmat", "t7arsorl", "t7arssci", "t7arswrt")
tch_eval_ncog_7 <- c("g7wrkhrd", "g7attent", "g7organz", "m7wrkhrd", "m7attent", "n7wrkhrd", "n7attent")
child_eval_ncog_7 <- c("c7sports", "c7trywt", "c7home", "c7angry", "c7angry", "c7likrd", "c7lonly", "c7flgood", "c7worth", "c7able", "c7stops", "c7noplan", "c7cherpa", "c7sdqint")
child_eval_cog_7 <- c("c7sdqmtc", "c7sdqrdc")
par_eval_7 <- c("p7learng")
par_vars_7 <- c("p7attenp", "p7ofttlk", "p7hmemp")
```
