...
ecls_clean <- ecls_clean %>%
  mutate(
    ...
    across(
      matches("^[gnt]7(behind|tlkout|passiv|relwel|disrup|absent|tardy|honors|organz|grammar|gather|wrtvar|style|analyo|creato|attent|orgdat|write|problm|presen|designn|scconc|scprob|talkab|proofs|apply|repres)"),
      ~ if_else(.x <= 0, NA_real_, .x)
    ),

    # Fix negatives or zero for grade 6 academic/cognitive variables
    across(
      matches("^[gnt]6(alg|rtmth|abil|divide|strat|plcvl|shape|rtlang|promot|speak|raloud|info|rdfln|expost|mltstr|reflwr|rtskil|expln|clssfy|comsc|sciprd|princp)"),
      ~ if_else(.x <= 0, NA_real_, .x)
    ), 
    
    across(
      matches("c7(home|angry|likrd|lonly|flgood|worth|able|stops|noplan|closcl|clostc|enjoy|howfar|clslik|grdyou|grdpar|mthgd|likmth|grdeng|enjrd|enjmth|clsoth|clscar|frover|gofrnd|frout|talk)"), 
      ~ if_else(.x <= 0, NA_real_, .x)
    ), 
    
    across(
      matches("p[1-7](hlphwk|librar|music|museum|attenp|spank)")
    )
  )
...
ecls_clean <- ecls_clean %>%
  mutate(
    # 1. Z-score transformation for matching variables
    across(
      matches("^[gnt]7(behind|tlkout|passiv|relwel|disrup|absent|tardy|honors|organz|grammar|gather|wrtvar|style|analyo|creato|attent|orgdat|write|problm|presen|design|scconc|scprob|talkab|proofs|apply|repres)"), 
  ...
  mutate(
    across(
      matches("^[gnt]7(passiv|attent|behind)_z"), 
      ~ -.
    )
  ) 
...
ecls_clean <- ecls_clean %>%
  mutate(
    noncog_7 = rowMeans(pick(matches("^[gnm]7(tlkout|relwel|disrup|absent|tardy|passiv|wrkhrd|attent)_z")), na.rm = TRUE),
    cog_7 = rowMeans(pick(matches("^[gnm]7(behind|organz|grammar|gather|wrtvar|style|analyo|creato|orgdat|write|problm|presen|design|scconc|scprob|talkab|proofs|apply|repres)_z")), na.rm = TRUE)
  )
```

# 5th grade cog noncog scores 
```{r}
ecls_clean <- ecls_clean %>%
  mutate(
    ...
    across(matches("learn|extern|intern|contro|interp|angry|lonly|stops|noplan|clslik|closcl|clostc|enjoy|grdyou|mthgd|engbst|likmth|grdeng|enjrd|enjmth|clsoth|clscar|frover|gofrnd|frout|talk"), 
           ~as.numeric(.))
  ) %>%
  mutate(
    across(
      matches("^[gntm]6(learn|extern|intern|contro|interp|alg|rtmth|abil|divide|strat|plcvl|shape|rtlang|abil|promot|speak|raloud|info|rdfln|expost|mltstr|reflwr|rtskil|expln|clssfy|comsc|sciprd|princp)"), 
      ~(. - mean(., na.rm = TRUE)) / sd(., na.rm = TRUE), 
      .names = "{.col}_z"
    ),
    across(
...
ecls_clean <- ecls_clean %>%
  mutate(
    noncog_6 = rowMeans(pick(matches("^[t]6(learn|extern|intern|contro|interp)_z")), na.rm = TRUE),
    cog_6 = rowMeans(pick(matches("^[gnm]6(alg|rtmth|abil|divide|strat|plcvl|shape|rtlang|abil|promot|speak|raloud|info|rdfln|expost|mltstr|reflwr|rtskil|expln|clssfy|comsc|sciprd|princp)_z")), na.rm = TRUE)
  )
...
ecls_clean <- ecls_clean %>%
  mutate(
    ...
    child_social_7 = rowMeans(pick(matches("c7(closcl|clostc|enjoy|clslik|clsoth|clscar|frover|gofrnd|frout|talk)_z")), na.rm = T)
  )
```
...
covar_list <- "p1hfamil + w1raceth + wkincome + p1hmemp + p1hdemp + p1numsib + w1momed + w1daded + w1dadscr + w1momscr + p1firkdg + p1disabl + sch_math_mean_1 + sch_rdg_mean_1"
...
for (i in 1:7) { 
  # run model
  model_output <- run_iv(paste0("c", i, "r4mscl"), paste0(paste0("age_test_", i, "+"), covar_list))
...
for (i in 1:7) { 
  # run model
  model_output <- run_iv(paste0("c", i, "r4rscl"), paste0(paste0("age_test_", i), covar_list))
...
balance_df_interval <- balance_df_interval %>%
  mutate(c7_college = as.numeric(c7howfar == 7))

model_honors <- run_iv("p7chlboo", paste0("age_test_7 + ", covar_list))
...
ma <- run_ols("c7howfar", paste0("age_test_7+", covar_list))[[1]]
summary(ma)
