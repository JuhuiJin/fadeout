...
data[["social_1"]] <- gamma_ncog * data[["social_k"]] + (1-gamma_ncog) * (data[["mean_social_k"]] + data[["k_1_diff"]]) + rnorm(n_new, mean = 0, sd = sigma)
...
generate_data <- function(gamma_cog, gamma_ncog){
  for (skill in c("test_", "social_")) {
    ...
    data[[curr_col]] <<- (((gamma + grade*0.015) * (data[[prev_col]]))^phi + ((1-gamma - grade*0.015) * (data[[paste0("mean_", skill, "k_0")]] + data[["white"]] * grade * 0.4))^phi)^(1/phi)  + rnorm(n_new, mean = 0, sd = sigma)
    }
    for (grade in 4:8){
    prev_col <- paste0(skill, grade - 1)
    curr_col <- paste0(skill, grade) 
...
graph_scores <- function(...) { 
  ...
  ggplot() + 
  geom_line(data = star_long %>% filter(!!covars), aes(x = Grade, y = mean_test, color = "Test Score (STAR)")) +
  geom_line(data = data_long %>% filter(!!covars), aes(x = Grade, y = mean_test, color = "Test Score (Simulated)")) + 
  geom_line(data = data_long %>% filter(!!covars), aes(x = Grade, y = mean_social, color = "Non-Cognitive Score (Simulated)")) +
  theme_minimal() + 
  labs(
    x = "Grade", 
    y = "Mean Score", 
...
graph_scores(black == 1, lunch == 1, treat_flow == "to_3")

covars <- quos(black == 1, lunch == 1, treat_flow == "to_3")
covars
```
