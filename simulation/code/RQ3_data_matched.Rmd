...
covars_min <- setdiff(covars, c("smalli", "smallk"))
...
# Example usage
regress("test_k", "smallk", covars_min, data)
regress("avsumk", "smallk", covars_min, star)
```
...
test_mean_k <- mean(subset(data, smallk == 0)[["test_k"]], na.rm = T)
...
generate_data <- function(gamma_cog){
  ...
  data[[curr_col]] <<- (gamma_cog * (data[[prev_col]])^phi + (1-gamma_cog) * ((test_mean_k)^phi))^(1/phi) + rnorm(n_new, mean = 0, sd = sigma)
  }
  for (grade in 2:8){ 
    prev_col <- paste0("social_", grade - 1) 
    curr_col <- paste0("social_", grade) 
    data[[curr_col]] <<- (gamma_ncog * (data[[prev_col]])^phi + (1-gamma_ncog) * ((test_mean_k))^phi)^(1/phi) + rnorm(n_new, mean = 0, sd = sigma)
  }
}
